...
echo "fill efficiency and puriry: ${18}"
echo "fill efficiency and puriry with IP: ${19}"
echo "with model to compare: ${20}"

root -x -l -b -q "task/plotSV.cxx(\"$DATA_FILE_PATH\", \"$SIM_FILE_PATH\", \"$SOURCE_SET\", \"$DATA_SET\", \"$MC_SET\", \"$TRIGGER_NAME\", \"$FIGURE_NAME\", \"$SUFFIX\", $INCLUSION_TYPE, $USE_LOG_SCALE, $PROCESS_DATA, $PROCESS_MCD, $PROCESS_MCP, $PROCESS_TEST, $PROCESS_2PRONG, $PROCESS_3PRONG, $FILL_N1, $FILL_EFFI, $FILL_EFFI_IP, $WITH_MODEL)"
